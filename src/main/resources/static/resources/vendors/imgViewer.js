...
//		self.zimg = $("<img />", {"src": self.img.src}).appendTo("body").wrap("<div class='viewport' />");
			//TODO : 해당 플러그인의 body에 append 시키는것을 본연의 객체에 전달
			$img.parent().addClass("viewport");
			self.zimg =	self.img;
 ...
 */
                ...
                //TODO : 부드럽게 움직이는 작동을 위해  transition: "100ms ease" 옵션추가
				$(this.zimg).css({
				...
				//TODO : 부드럽게 움직이는 작동을 위해  transition: "100ms ease" 옵션추가
				$(this.zimg).css({transition: "100ms ease" , transform: "translate(" + xt + "px," + yt + "px) scale(" + zoom + "," + zoom + ")" });
                //TODO : 해당 슬라이더가 투핑거 액션 줌이 작동중인지 아닌지 체크한다.
				if(xt == 0 && yt == 0){
                    $(this.zimg).attr("slider",true);
				}else{
                    $(this.zimg).attr("slider",false);
				}
/*
