(function($, window, document) {
    ...
    Plugin.prototype = {
        init: function() {
            ...
            elem.on('touchstart.' + pluginName, function(ev) {
                self.inProgressTouchstart = (self.inProgress || (!self.options.allowPtrWhenStartedWhileScrolled && (self.options.scrollingDom || $(document)).scrollTop() > 0));
            ...
            elem.on('touchmove.' + pluginName, function(ev) {
                ...
                if (self.top > 1 && self.top <= self.options.maxPullThreshold) {
                    ...
                    self.topTranslation = self.topTranslation > 50 ? 50 : self.topTranslation;
                    self.requestAnimationFrame(function() {
    ...
    $(document).on('ready',function(){
        const top =$('.top').outerHeight() + "px";
        $('.content').css({'padding-top': top})

        let spinnerAni = lottie.loadAnimation({
            container: document.getElementById('ptr-light-spinner'),
            renderer: "svg",
            loop: true,
            path: "/lotties/ani_refresh.json",
        });
    });
